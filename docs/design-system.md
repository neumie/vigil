...
- **Do** cap at 4–5 segments (sidebar filter: Needs / Active / Queue / Inbox). **Don't** build full-width tab bars with icons; **don't** let labels wrap or ellipsize — shorten the label instead.
...
- Project identity is a **2 × 24px vertical rail** at the row's 3px left inset, using the project's configured color (`--text-3` when unconfigured). It must not be another dot: the round 8px marker on line 1 belongs exclusively to lifecycle status, and two nearby dots falsely read as two statuses.
- Line 2 (2px below): 11px `--text-2` meta — project tag, then ONE of: the mini assessment-verdict chip, a compact Agent/Manual ownership marker, or document icon + server-cached planning/ticket progress. Planning has three truthful labels: **Planning** (session exists, no runnable spec detected), **Plan ready** (spec/PRD exists, no explicit queue), or **X of Y tickets complete** (combined local `issues/*.md` + associated GitHub issues; complete = total − open). Once ticket data exists, that progress marker wins in every lifecycle state—including Review and Archive—so completion does not disappear after execution starts.
...
- A 6px circular project-color swatch is allowed beside the selected project in the toolbar filter and beside the project slug in Item detail because neither location places it beside the lifecycle dot. The visible slug remains the accessible identity; color is decorative and never the sole project cue.
...
- **Effort control:** Run setup places a native Effort select after Model: Default, Low, Medium, High, Extra high, plus Max for Claude. Default clears the Item override; switching from Claude Max to Codex clears Max. The quiet caption says “Used by Start loop” so ordinary solver execution is not falsely implied.
