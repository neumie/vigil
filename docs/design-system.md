...
- **Organization menu:** the toolbar's group icon opens a radio menu with **Flat list** / **Group by project**. The active project-grouping state accents the icon; the preference is local and changes only presentation—project scope filtering and server queries are untouched. Grouped mode inserts flat 30px project headers (colored slug + Item count) above the same dense rows; it does not create rounded project cards.
