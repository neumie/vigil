...
- Custom webkit scrollbar inside panes: 10px gutter, transparent track, thumb `rgba(255,255,255,0.14)` inset 3px via `background-clip: content-box` (hover `0.24`), radius 999. Every persistent native scroll container uses `scrollbar-gutter: stable` so crossing the overflow threshold never narrows content or slides text/controls. This covers sidebar pages/lists/sheets, evidence/plan wells, background-terminal rows, and both Run Context panes. The terminal uses helm's own overlay scrollbar per §3.14 (full-pane track, 7px pill, the brighter `--term-scroll-thumb` pair) and therefore does NOT reserve a native gutter.
- **Don't** leave a default (light) scrollbar on any pane surface; **don't** add `overflow: auto` without reserving a stable gutter unless the surface owns an overlay scrollbar.
...
- **Popover — editorial terminal list**: §3.8 panel (background `--chrome`, `--hairline`, radius `--radius-lg`, `--shadow-1`, z 40), fixed width 320 (viewport-clamped) and max-height `min(480px, 100vh - 48px)`, anchored `menu-end` under the button; rows scroll inside a stable gutter while the header stays fixed. Its 12/600 header places "Background terminals" left and a quiet tabular count right—no second chip. One row per background terminal, park order.
  ...
  - Three explicit operations remain: clicking the title target **opens and focuses the terminal while it remains parked** (no tab button, count and persisted `parked` flag unchanged); quiet icon-only **⇥** (28px, tooltip/accessible name "Move to tabs and open") moves it to the strip end and opens it; quiet **×** closes through the grace path. Undo after close returns it to the background list, not to a tab.
...
- **Progress variant:** motion is clockwise: top-left → top-right → middle-right → bottom-right → bottom-left → middle-left. One dot advances every 200ms, one 1.2s cycle; opacity fades from the bright neutral head into the quiet grayscale tail.
