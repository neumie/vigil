...
- **Run evidence and run setup are inline groups on Item detail** (the retired Work hub and its pushed pages are in the appendix), ordered per lifecycle state by `detail-state.ts` — decision content first: Activity keeps only compact current facts visible (Started / State / Loop / Round and any distinct summary); its entire newest-first event timeline is hidden at rest behind **"Show history" / "Hide history"** — never leak a teaser list before the user asks. Log is append-only output normalized to newest-first; the two newest non-empty messages remain visible at rest in `.log-well`, then "Show all" / "Show less" reveals the available log. Solve input uses a §3.20 disclosure. Run setup keeps a one-line effective-selection summary readable at rest (12 `--text-1`, "Claude Code · model · effort · Worktree") above a "Change setup" disclosure holding the four pickers. Planned items with tickets get a minimal Plan section whose neutral trailing header chip carries `X of Y complete`; it has no fact rows. **Status is redundant with ticket progress, Spec is represented by Plan documents, and Plan documents belongs beside Task in the shared destination group — none gets a row inside Plan.** A planned Item with no tickets omits the empty Plan section entirely.
...
The sanctioned way to keep heavy evidence in place without a subpage (`Disclosure` in `app/src/renderer/sidebar/ui.tsx`) — the promotion of the old ad-hoc `.detail-disclosure` + `useState` pattern. Solve input and Run setup use Disclosure because they have no useful collapsed content. Activity uses the same bare control but renders **no history rows** until "Show history" is activated. Log is the deliberate preview exception: its two newest messages remain visible at rest.

- Shape: a quiet toggle **button** in the `.detail-disclosure` style (11/600 `--accent`, min-height 24) with `aria-expanded`/`aria-controls` and a sentence-case **verb pair** ("Show history" / "Hide history", "Show all" / "Show less", "Change setup" / "Hide setup"); revealed content follows in the group's flow.
...
- `Disclosure` children render only while open. The Log exception always renders exactly its two-message preview while collapsed; expanding replaces it with all currently available messages, still newest-first.
- `defaultOpen` is a **mount-time default only**, driven by `detail-state.ts`'s per-state `open` flag (today: the log on failed). It is never re-applied on a status flip — a mid-read status change must not collapse a section the user opened or pop one open under their pointer.
- Long text inside a disclosure or log preview lives in a `--well` scroll box (`.log-well` 240px, `.plan-well` 320px) with text selection enabled (§4) and keyboard focus (`tabIndex` 0).
...
| What | Was | Now |
| --- | --- | --- |
| Run evidence (timeline, log, solve input) | pushed Run details page behind a Work-group nav row, log behind a further Show-log toggle | inline detail groups (§3.15), log/input as §3.20 disclosures; log auto-open on failed |
| Run setup | pushed Run setup page behind a nav row | inline summary-at-rest + "Change setup" §3.20 disclosure |
| Work hub group | "Work" group with Run details / Task / Plan nav rows | Task + Plan documents as one shared destination group; ticket progress in the Plan header |
