...
- Shape: the Section-header owns one trailing text **button** (`.disclosure-action`): 11/500, `--text-1`, transparent at rest, 24px hit target, `--radius-sm`; hover uses `--fill-subtle` + `--text-0`, and expanded state stays `--text-1`. The heading supplies context, so copy stays short: **Show / Hide** for evidence and **Change / Done** for editable setup. No pill, `+ / −`, chevron, or separate toggle row.
- It is a section-level action, NOT a 28px fact/action row or 36px nav row. §3.15's two-pitch law stays intact because the control occupies the existing header slot rather than adding another row.
