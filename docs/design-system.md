...
- **Popover — editorial terminal list**: §3.8 panel (background `--chrome`, `--hairline`, radius `--radius-lg`, `--shadow-1`, z 40), fixed width 320 (viewport-clamped) and max-height `min(480px, 100vh - 48px)`, anchored `menu-end` under the button; rows scroll inside a stable gutter while the header stays fixed. Its 12/600 header places "Background terminals" left and a quiet tabular count right—no second chip. One row per background terminal, park order.
  - Row: minimum 52px, three-column grid, 4px inter-row gap. The large two-line **Open** target contains a fixed 10px shared §3.21 indicator slot spanning both lines, title 13/500 `--text-0` single-line ellipsis (exited rows: `--text-1`), and state beneath at 11 `--text-2` tabular ("Running" / "Exited (code)"). Pi's OSC state alone drives the indicator; arbitrary terminal output does not. The Open target's accessible name includes title, Running/Exited, and the OSC agent state so its custom action label never hides visible status from assistive technology. Hover and the currently-open parked row use the same square, panel-edge-to-edge `--fill-subtle` stripe as sidebar rows (§2.3); only the ⇥/× controls are rounded.
...
- **Copy**: menu item "Move to background", header "Background terminals", states "Running" / "Exited (n)", ⇥ tooltip/accessible name "Move to tabs and open" — sentence case per §5.
