...
- Line 2 (2px below): 11px `--text-2` meta — project tag, then ONE of: the mini assessment-verdict chip, a compact Agent/Manual ownership marker, or document icon + server-owned planning readiness while an Inbox/Queue/Active Item has an interactive plan. Planning readiness has three truthful labels: **Planning** (session exists, no runnable spec detected), **Plan ready** (spec/PRD exists, no explicit queue), or **N tickets ready** (local `issues/*.md` and/or GitHub `loop(<plan>)` issues exist). The readiness marker wins during that pre-run phase; ownership wins after execution starts.
