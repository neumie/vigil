...
- Line 2 (2px below): 11px `--text-2` meta — project tag, then ONE of: the mini assessment-verdict chip, a compact Agent/Manual ownership marker, or document icon + **Planned** while an Inbox/Queue/Active Item has a prepared interactive plan (14px outline icon + label). Planned wins during that pre-run phase; ownership wins after execution starts.
