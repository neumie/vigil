...
- **Agent-running tabs:** Pi's explicit OSC 9;4 protocol owns this state (`3` active, `0` clear; active repeats every second). A running tab appends the compact visible word **Running** with the standard 6px `--accent` live pulse; the tab's accessible name also includes Running. Repeated keepalives are idempotent. Park/restore preserves the state because the process remains attached; close or PTY exit clears it. Never infer running/completion from ordinary output, silence, process names, shell prompts, or tab titles.
