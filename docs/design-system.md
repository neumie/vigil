...
### 3.20 Inline disclosure

The sanctioned way to keep heavy evidence in place without a subpage (`Disclosure` in `app/src/renderer/sidebar/ui.tsx`) — the promotion of the old ad-hoc `.detail-disclosure` + `useState` pattern. Solve input and Execution setup use Disclosure because they have no useful collapsed content. Activity puts its disclosure control in the Section-header trailing slot and renders **no body** until "Show history" is activated; expansion shows result summary + lifecycle events. Log is not a disclosure: it is always fully present inside its bounded scroll well.

- Shape: a compact neutral toggle **button** in the `.detail-disclosure` style (11/600 `--text-1`, 28px, 6px radius, `--fill-subtle` → `--fill-raised`) with `aria-expanded`/`aria-controls`, a sentence-case **verb pair** ("Show history" / "Hide history", "Change setup" / "Hide setup"), and a trailing `+` / `−` state mark. Revealed content follows in the group's flow.
- It is a button, NOT a 36px nav row: §3.15's two-pitch law stays intact — 28 = read/act in place, 36 = go somewhere. A disclosure never carries a directional chevron or uses accent color as its only affordance.
- Content **snaps** open and closed — never animate height (§2.5). Reduced motion is automatically satisfied.
- `Disclosure` children render only while open. The always-expanded Log bypasses `Disclosure` entirely.
- `defaultOpen` is a mount-time-only primitive for exceptional disclosure stories/callers; status changes must never re-apply it and override the user's current choice.
- Long text inside a disclosure or the always-expanded Log lives in a `--well` scroll box (`.log-well` 240px, `.plan-well` 320px) with text selection enabled (§4) and keyboard focus (`tabIndex` 0).
- **Don't** use a disclosure where the content is short enough to just show (a fact row costs less than a toggle); **don't** nest disclosures; **don't** use one to hide a page's primary decision content — it is for evidence, not for the decision.
...
| What | Was | Now |
| --- | --- | --- |
| Run evidence (timeline, log, solve input) | pushed Run details page behind a Work-group nav row, log behind a further Show-log toggle | inline detail groups (§3.15); Activity/input use disclosures, Log is always expanded |
| Execution setup | pushed Run setup page behind a nav row | shared agent/model/effort/workspace summary + "Change setup" §3.20 disclosure |
