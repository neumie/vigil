...
- **Popover**: §3.8 panel (background `--chrome`, `--hairline`, radius `--radius-lg`, `--shadow-1`, z 40), fixed width 300, anchored `menu-end` under the button. Header "Background terminals" in the Label type style. One row per background terminal, park order.
  - Row: 32px pitch, three-column grid. The large **Open** target contains a fixed 6px shared §3.21 indicator slot + title 13 `--text-0` single-line ellipsis (exited rows: `--text-1`) + state 11 `--text-2` tabular ("Running" / "Exited (code)"). Pi's OSC state alone drives the indicator; arbitrary terminal output does not. A currently-open parked row gets the same subtle fill as hover.
  - Three explicit operations: clicking the title target **opens and focuses the terminal while it remains parked** (no tab button, count and persisted `parked` flag unchanged); compact **Tab** moves it to the strip end and opens it; **×** closes through the grace path. Undo after close returns it to the background list, not to a tab.
  - Dismiss: Open/Tab close the popover; outside click or Esc closes it and returns focus to the stack button. ↑/↓ move between Open targets; Tab reaches each row's Tab and × controls normally.
...
- **Copy**: menu item "Move to background", header "Background terminals", states "Running" / "Exited (n)", action label "Tab" with tooltip/accessible name "Move to tabs and open" — sentence case per §5.
